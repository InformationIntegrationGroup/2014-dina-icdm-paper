...
%The Karma execution time is the sum of user time and system processing time. We observed that the system processing time is approximately 20\% of the total execution time. 
%Did you mean the 9:30mins and 22:30mins both include user and system time? This sentence is not clear

%When using Excel it took 40:20 minutes to complete the same set of trials. 
...
%We observed a 42.14\% reduction in time taken for the iterative process of data preparation by using Karma to process six data files. 
%user time or system?

%To emphasize the potential of our results, we could extrapolate our dataset to have 50 files instead of 6 files in the mode of transportation data.
%You need to provide a real world scenario. Extrapolating your datasets doesn't make much sense. It feels like you are making up something to prove a case. How about this:
%Assuming the user is collecting datasets for 50 days, at the end of each day, the user wants to do this data mining tasks on the newly collected data and hence he/she need to apply Karma/Excel again.

%The total time spent iterating over each file using Excel could be approximately 336 minutes.
%This still reads like you want the user to sit down and spend 336min on Excel or 118min on Karma. I think you meant the accumulated time to process all the 50 files (when they are processed individually). You need to make this clear.
...
%Here the real benefit 
%"real" benefit implies there are some fake benefit...rewrite
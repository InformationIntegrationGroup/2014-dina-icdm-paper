...
After we have published the data from the addDFT service, we perform a join operation with the LocationProbe data that was published previously. Starting from the topmost class in the model, i.e., AccelerometerReading, we select the `Augment Data' option, as shown in figure \ref{fig:augment_data_menu}, from the menu that appears after clicking the AccelerometerReading bubble.
